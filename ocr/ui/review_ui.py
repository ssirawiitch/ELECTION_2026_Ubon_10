...
# ─── Config ───────────────────────────────────────────────────────────────────
DATA_FILE      = Path("data/election_insights_final.json")
CSV_FILE       = Path("data/score_ocr_results.csv")
CROPS_DIR      = Path("yolo_score_crops/yolo_score_crops")  # ที่อยู่จริงของรูปตัด
BACKUP_FILE    = Path("data/election_insights_final.BACKUP.json")
CSV_BACKUP     = Path("data/score_ocr_results.BACKUP.csv")

st.set_page_config(
    page_title="ตรวจสอบ OCR | เขต 10 อุบลราชธานี",
    page_icon="🗳️",
    layout="wide",
)
...
.stat-num  { font-size: 1.8rem; font-weight: 700; color: #8e54e9; }
...
def make_backup(data):
    ...
    if not CSV_BACKUP.exists():
        # backup CSV ด้วย
        pd.read_csv(CSV_FILE, dtype=str, low_memory=False).to_csv(
            CSV_BACKUP, index=False, encoding="utf-8-sig"
        )

# ─── Collect items that need review ──────────────────────────────────────────
def collect_review_items(data):
    items = []
...
if "review_items"  not in st.session_state: st.session_state.review_items = collect_review_items(st.session_state.data)
...
# ─── Header ───────────────────────────────────────────────────────────────────
st.markdown("## 🗳️ ตรวจสอบ OCR คะแนนเลือกตั้ง — เขต 10 อุบลราชธานี")
...
<div class="stat-label">รายการต้องตรวจสอบทั้งหมด</div></div>""", unsafe_allow_html=True)
...
c3.markdown(f"""<div class="stat-box"><div class="stat-num" style="color:#e9c46a">{total - cur_idx:,}</div>
...
with right:
    ...
    • ข้อมูลจะถูกบันทึกอัตโนมัติทุกครั้งที่กดยืนยัน
    </div>
    """, unsafe_allow_html=True)
